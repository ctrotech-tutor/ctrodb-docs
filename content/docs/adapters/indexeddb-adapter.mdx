...
- Each collection is an object store with `id` as keyPath
- Auto-increment is handled by IndexedDB
- Indexes are created during migration (`onupgradeneeded`)
- A `_ctrodb_meta` store holds schema version and metadata
