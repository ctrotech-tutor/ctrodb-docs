...
Internal — collections are created by `Database.collection(){:ts}`.
...
Fetches a record by ID. Returns `undefined{:js}` if not found.
...
Returns a new `QueryBuilder{:ts}` for this collection.
