...
Throws `SchemaError{:ts}` on invalid config (no version, no collections, empty collection,
...
The `id{:ts}` field is exempt from the undefined-field check.
