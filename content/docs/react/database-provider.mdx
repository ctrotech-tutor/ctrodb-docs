...
`DatabaseProvider{:tsx}` makes the database instance available to all hooks in the tree.
...
The `useDatabase{:ts}` hook reads from context first, falling back to the default.
