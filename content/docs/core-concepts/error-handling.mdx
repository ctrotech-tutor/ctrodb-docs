...
- `CollectionNotFoundError(name, availableCollections?)` — for schema-constrained lookups
- `RecordNotFoundError(collection, id)` — for record-level lookups
- `QueryError(message)` — for malformed queries
